...
const { app, BrowserWindow, shell, ipcMain, dialog, desktopCapturer, session, Menu } = require('electron');
    ...
    const template = [
        {
            ...
            submenu: [
                { role: 'about' },
                { type: 'separator' },
                // Intentionally omit 'hide' and 'hideOthers' to free Cmd+H
...
app.whenReady().then(async () => {
    // Set macOS menu without 'Hide' to free Cmd+H
    setAppMenu();

    // Setup capture policy: only allow loopback on Windows; require explicit gating flag
    session.defaultSession.setDisplayMediaRequestHandler((request, callback) => {
        try {
            const allowCapture = process.env.PICKLEGLASS_ALLOW_CAPTURE === 'true';
            if (!allowCapture) {
                return callback({});
            }
            if (process.platform === 'win32') {
                ...
                // Non-Windows: do not grant loopback audio by default
                desktopCapturer.getSources({ types: ['screen'] }).then((sources) => {
                    if (!sources || sources.length === 0) return callback({});
            ...
            console.error('Error in setDisplayMediaRequestHandler:', e);
            callback({});
        }
    });
