...
async function createSTT({ apiKey, language = 'en', callbacks = {}, usePortkey = false, portkeyVirtualKey, ...config }) {
  ...
  if (usePortkey && !process.env.PORTKEY_API_KEY) {
    throw new Error('PORTKEY_API_KEY environment variable is required when usePortkey=true');
  }
    ...
    const attempt = async () => {
      if (!usePortkey) {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Portkey API error: ${response.status} ${response.statusText} ${text}`);
        }
