...
export const apiCall = async (path: string, options: RequestInit = {}) => {
  ...
  // Simple in-memory TTL cache for GETs
  const isGet = (defaultOpts.method || 'GET').toUpperCase() === 'GET'
  const cacheKey = isGet ? `${url}|${JSON.stringify(defaultOpts.headers || {})}` : null
  const ttlMs = 10_000
  ;(globalThis as any).__PG_API_CACHE__ = (globalThis as any).__PG_API_CACHE__ || new Map<string, { t:number, v:Promise<Response> }>()
  const cache: Map<string, { t:number, v:Promise<Response> }> = (globalThis as any).__PG_API_CACHE__

  if (isGet && cacheKey) {
    const hit = cache.get(cacheKey)
    const now = Date.now()
    if (hit && now - hit.t < ttlMs) {
      return hit.v.then(async r => new Response(await r.clone().blob(), r))
    }
    const req = fetch(url, defaultOpts)
    cache.set(cacheKey, { t: now, v: req })
    return req
  }
